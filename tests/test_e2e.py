...
class TestEndToEnd:
    def test_full_workflow(self, app, client):
        ...
        with app.app_context():
            project = ProjectService.get_all()[0]
            pid = project.id

        # 2. Verify project detail page has sidebar and breadcrumbs
        ...
        response = client.post(
            ...
            data={
                ...
                "environment": "Python 3.10, SQLite",
            },
            follow_redirects=True,
        )
        assert response.status_code == 200
        ...
        response = client.put(
            ...
            json={
                "data": {
                    ...
                    "edges": [
                        {"id": "e1-2", "source": "1", "target": "2", "label": "REST"},
                    ],
                },
            },
        )
        ...
        assert len(data["diagrams"][0]["edges"]) == 1
        assert data["api_endpoints"][0]["path"] == "/api/e2e/test"
        assert data["api_endpoints"][0]["method"] == "POST"
        ...
        assert "## Project Plans" in md
        assert "## Test Plans" in md
        assert "## Diagrams" in md
        assert "## API Endpoints" in md
        assert "E2E Requirement" in md
        assert "developer" in md
        assert "`POST /api/e2e/test`" in md

    def test_sidebar_navigation_present(self, app, client):
        """Verify sidebar appears on all project-scoped pages."""
        with app.app_context():
            p = ProjectService.create(name="Nav Test")
            pid = p.id
        ...
        for url in pages:
            ...
            assert b"sidebar-nav" in response.data, f"No sidebar on {url}"
            assert b"breadcrumb" in response.data, f"No breadcrumb on {url}"

    def test_breadcrumb_on_nested_pages(self, app, client):
        """Verify breadcrumbs include proper hierarchy on detail pages."""
        with app.app_context():
            p = ProjectService.create(name="Breadcrumb Test")
            pid = p.id
        ...
        response = client.get(f"/projects/{pid}/documents/{doc_id}")
        assert response.status_code == 200
        html = response.data.decode("utf-8")
        assert "Projects" in html
        assert "Breadcrumb Test" in html
        assert "Documents" in html
        assert "Test Req" in html

    def test_dashboard_shows_artifact_counts(self, app, client):
        """Verify the dashboard shows document/diagram/endpoint counts."""
        with app.app_context():
            p = ProjectService.create(name="Count Test")
            pid = p.id
        ...
        assert response.status_code == 200
        html = response.data.decode("utf-8")
        assert "Count Test" in html
        assert "2 docs" in html
        assert "1 diagrams" in html
        assert "0 endpoints" in html
